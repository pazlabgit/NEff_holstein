...
rumen_ids_neff <-  read_tsv("metadata/metadata_lefse.txt")%>% 
...
#join rumen_sampleid and holstein_ids_neff dataset
neff_rumen <- left_join(rumen_sampleid, rumen_ids_neff, by="SampleID") %>% 
  select(Phenotype, SampleID) #reorder columns
...
colnames(neff_rumen_t) <- as.character(unlist(neff_rumen_t[2,])) #rbind needs same headers
...
#create phenotype dataset
feces_ids_neff <-  read_tsv("metadata/metadata_lefse.txt")%>% 
  select("#SampleID", Site, Phenotype) %>% 
  rename(SampleID="#SampleID")%>% 
  slice(2:23) %>% 
  filter(Site=="Feces") %>% 
...
#make second row header
colnames(neff_feces_t) <- as.character(unlist(neff_feces_t[2,])) #rbind needs same headers
...
#create file
write.table(feces_neff_hol_lefse, file="lefse/asv_lda_input_files/lefse_asv_input_feces", sep="\t", col.names=F, row.names=T, quote=F)
```
